...
def code_search_tool(
    ...
    Args:
        ...
        extensions: List of file extensions to include (e.g. [".py", ".js"]).
        k: Number of results to return.
        remove_duplicates: Whether to remove duplicate file results (default False).
        min_score: Minimum score threshold to filter out low-quality matches (default 0.0).
    ...
    # Initialize index if it doesn't exist and repo_path is provided
    if not index_exists and repo_path is not None:
        init_result = initialize_code_search(
            repo_path=repo_path,
            save_dir=save_dir,
    ...
    # Filter results: remove duplicates and apply score threshold
    if search_result["status"] == "success":
        filtered_results = []
        seen_files = set()
        ...
        for result in search_result["results"]:
            ...
            if remove_duplicates and result["file"] in seen_files:
                continue
